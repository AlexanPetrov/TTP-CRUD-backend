...
app.post("/school", async (request, response, next) => {
	try {
		...
		const newStudent = await pool.query(
			"INSERT INTO student (name, campus_id, gpa, studenturl) VALUES ($1,$2,$3,$4)",
...
app.post("/school/campus", async (request, response, next) => {
	try {
		...
		const newCampus = await pool.query(
			"INSERT INTO campus (campusName, campuslocation, campusimageurl, campusdescription) VALUES ($1,$2,$3,$4)",
...
app.get("/school", async (request, response, next) => {
	try {
		...
		response.send(allStudents.rows);
	} catch (err) {
		console.error(err.message);
	}
});
